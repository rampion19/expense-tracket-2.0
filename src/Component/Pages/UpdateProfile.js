...
export const UpdateProfile = () => {
        ...
        async function fetchProfile() {
            try {
                const res = await fetch(
                    "https://identitytoolkit.googleapis.com/v1/accounts:lookup?key=AIzaSyDZnqHwYDKWIazKAZlYNcW3CnY3LV1aWAU",
                    {
                        method: "POST",
                        body: JSON.stringify({
    ...
    const profileSubmitHandler = async (event) => {
        ...
        try {
            const res = await fetch(
                "https://identitytoolkit.googleapis.com/v1/accounts:update?key=AIzaSyDZnqHwYDKWIazKAZlYNcW3CnY3LV1aWAU",
                {
                    method: "POST",
                    body: JSON.stringify({
                        idToken: JSON.parse(localStorage.getItem("idToken")).idToken,
